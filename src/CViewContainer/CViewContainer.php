...
class CViewContainer {
    ...
    public function SetTitle($value) {
        /*
         * $append = CSiteshop::Instance()->config['title_append'];
          $separator = CSiteshop::Instance()->config['title_separator'];
    ...
    public function Render($region = 'default') {
        if (!isset($this->views[$region]))
            return;
        if (isset($this->views[$region]['order']) && ($this->views[$region]['order'] > 0) ) {
    ...
    public function AddIncludeToRegion($region, $file = null, $variables = array(), $order = 0) {
        ...
        if (empty($file))
            throw new Exception(t('View filename is empty.'));
    ...
    public function Add($args = array()) {
        ...
        foreach ($args['regions'] as $val) {
            switch ($val['type']) {
                case 'string': $this->AddStringToRegion($val);
                    break;
                case 'include': $this->AddIncludeToRegion($val);
                    break;
                case 'function': $this->AddFunctionToRegion($val);
                    break;
