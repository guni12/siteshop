...
class CFormElement implements ArrayAccess {
    ...
    public function GetHTML() {
        ...
        $size = " size=30";
        $space = isset($this['space']) && $this['space'] ? '<br />' : null;
        ...
        if ($this['type'] == 'submit') {
            return "$space<span class='submitLayout'><input id='$id'{$type}{$class}{$name}{$value}{$autofocus}{$readonly} /></span>";
            ...
            return "<span class='buttonLayout'><input id='$id'{$type}{$class}{$name}{$value}{$autofocus}{$readonly}{$placeholder} /></span><br />";
        } else if ($this['type'] == 'textarea') {
            return "<span class='textareaLayout'><label for = '$id'>$label</label></span><textarea id = '$id'{$type}{$class}{$name}{$autofocus}{$readonly}>{$onlyValue}</textarea>";
            ...
            return "<span class='layout'><label for='$id'>$label</label></span><input id='$id'{$type}{$class}{$name}{$size}{$value}{$autofocus}{$readonly} />{$messages}";
        }
    }

    /**
     * Get HTML code for an element. 
     *
     * @returns HTML code for the element.
     */
    public function GetHTML_Twit() {
        ...
        if ($type && $this['type'] == 'submit') {
            return "<input id='$id'{$type}{$class}{$name}{$value}{$autofocus}{$readonly} /><br />";
...
/**
 * A helpclass to handle textfields
 * 
 * @package SiteshopCore
 */
...
/**
 * A helpclass to handle textareas
 * 
 * @package SiteshopCore
 */
...
/**
 * A helpclass to handle textfields with bootwiitter
 * 
 * @package SiteshopCore
 */
...
/**
 * A helpclass to handle textareass with bootwitter
 * 
 * @package SiteshopCore
 */
...
/**
 * A helpclass to handle hidden formelements
 * 
 * @package SiteshopCore
 */
...
/**
 * A helpclass to handle password textfields
 * 
 * @package SiteshopCore
 */
...
/**
 * A helpclass to handle password textfields with bootwitter
 * 
 * @package SiteshopCore
 */
...
/**
 * A helpclass to handle submit-buttons
 * 
 * @package SiteshopCore
 */
...
/**
 * A helpclass to handle buttons with bootwitter
 * 
 * @package SiteshopCore
 */
 ...
 * @package SiteshopCore
 */
class CFormElementSpace extends CFormElement {
...
*/
      ...
      if($request) {*/// bortkommenterat till hit
